...
namespace imageOutliner {
	...
	:
			argc(argc), argv(argv), desc(NULL), bpo(NULL), bitMask(255), outlineColor(0xFFFFFFFF)
	{
	...
	void CommandLineOptionsBoost::validateOptions(){
		...
		if(vm.count("color") > 0){
			...
			if(colorString.length() == 8){ // try to parse binary mask
				...
				std::printf("using color \"%d, %d, %d, %d\" (%s).\r\n",
						(outlineColor) >> 24,
	...
	void CommandLineOptionsBoost::setAvailableCommandLineOptions(){
		...
		("output,o", po::value<std::string>(), "The generated file name, without extension.")
		("mask,m", po::value<std::string>(), "A bitmask telling wich pixels should be taken into account when outlining. Bits start at up neighbour and continue clockwise until upleft neighbour, exactly 8.")
		("color,c", po::value<std::string>(), "The RGBA color to be used as outline, provided in hex RGBA format, as in \"FFCC66FF\". If not provided by command line, defaults to white (FFFFFFFF).");
	}
